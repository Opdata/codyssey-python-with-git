...
class QuizGame:
    ...
    def play_quiz(self):
        ...
        is_checked_finish = False

        # 퀴즈 출제 및 입력 받고 정답 체크
        for quiz in selected_quizzes:
            IOHandler.display_quiz(quiz)
            inputed_answer = self.input_answer()
            self.check(quiz, inputed_answer)

        is_checked_finish = True

        if is_checked_finish:
            self.result(self.score)

    # 사용자가 정답을 입력 할 수 있다.
    def input_answer(self) -> int:
        return IOHandler.select_answer()
    ...
    def check(self, quiz: Quiz, inputed_answer: int):
        ...
        if check:
            ...
            IOHandler.display_wrong()

    # 모든 문제를 풀면 결과 표시 및 최고 점수 저장
    def result(self,score):
        IOHandler.display_result(self.answer_count, score)
        if score > self.high_score:
            save_state = FileHandler.save_score(score)
            IOHandler.display_save_error(save_state)
